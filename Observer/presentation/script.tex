\documentclass[11pt,a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage{geometry}
\usepackage{xcolor}
\usepackage{hyperref}
\usepackage{titlesec}
\usepackage{mdframed}

\geometry{margin=1in}

\titleformat{\section}{\large\bfseries\color{blue!80!black}}{}{0em}{}[\titlerule]

\title{Seminar Presentation Script: Observer Pattern}
\author{Group 52 \\ Nguyen Dinh Minh Huy (Student ID: 25125083)}
\date{\today}

\newenvironment{dialogue}[1]{
    \begin{mdframed}[linecolor=gray!40,backgroundcolor=gray!5,roundcorner=5pt,innerleftmargin=10pt,innerrightmargin=10pt,innertopmargin=8pt,innerbottommargin=8pt]
    \textbf{#1:}
}{
    \end{mdframed}
}

\begin{document}

\maketitle

\section{Presenters}
\begin{itemize}
    \item \textbf{Speaker A:} Member A
    \item \textbf{Speaker B:} Member B
\end{itemize}

\section{Presentation Flow \& Dialogue}

\subsection*{Slide 1: Title Slide (The Observer Design Pattern)}
\begin{dialogue}{Speaker A}
    Hello everyone, and welcome to Group 52's presentation. Today, my colleague and I will be discussing the Observer Design Pattern---a fundamental behavioral pattern that plays a critical role in decoupling state changes and notifications in modern software. Let's start with a real-world scenario.
\end{dialogue}

\subsection*{Slide 2: Real-world Problem (YouTube Subscription)}
\begin{dialogue}{Speaker B}
    Imagine you are building a video streaming platform like YouTube. Creators upload videos to their channels, and users subscribe to these channels. When a new video goes live, subscribers need to be notified immediately. 
    However, the challenge is that subscribers use different channels of notifications---some want an email alert, others want a mobile push notification, and some want updates on their Smart TV. Moreover, users subscribe and unsubscribe dynamically at runtime. How do we build this without tangling our codebase?
\end{dialogue}

\subsection*{Slide 3: Naive Solution (Without Pattern)}
\begin{dialogue}{Speaker A}
    In a naive solution, we might create concrete classes for each subscriber type, such as \texttt{EmailSubscriber} and \texttt{MobileSubscriber}. Our \texttt{NaiveYouTubeChannel} class then maintains separate lists for each of these concrete classes. 
    As you can see in this code snippet, when a video is uploaded, the channel iterates through the lists of email subscribers and mobile subscribers individually, calling their specific methods like \texttt{receiveEmailNotification} or \texttt{receivePushNotification}. While this works for simple scenarios, it is highly problematic.
\end{dialogue}

\newpage
\subsection*{Slide 4: Drawbacks of the Naive Solution}
\begin{dialogue}{Speaker B}
    This naive approach has three major flaws: \\
    First, \textbf{Tight Coupling}. The channel must know the exact concrete types of subscribers and their specific update methods. \\
    Second, it violates the \textbf{Open-Closed Principle (OCP)}. If we want to add a third subscriber type---say, \texttt{TVSubscriber}---we must modify the channel class, add a new vector list, and modify the notification loop inside \texttt{uploadVideo}. \\
    Lastly, it is \textbf{rigid and hard to scale} as the number of subscriber channels grows.
\end{dialogue}

\subsection*{Slide 5: Introduction to the Observer Pattern}
\begin{dialogue}{Speaker A}
    To solve this, we use the \textbf{Observer Pattern}. The intent of the pattern is to define a one-to-many dependency between objects. When the Subject---in our case, the YouTube Channel---changes state, all its dependents---our Subscribers---are notified and updated automatically. 
    The key is that the Subject only interacts with an abstract interface, not concrete classes. This cleanly separates the publisher from its subscribers.
\end{dialogue}

\subsection*{Slide 6: General Class Diagram}
\begin{dialogue}{Speaker B}
    Here is the general UML diagram of the pattern. We have:
    \begin{enumerate}
        \item The \textbf{Subject} interface, which defines methods to attach, detach, and notify observers.
        \item The \textbf{Observer} interface, which defines a simple \texttt{update()} method.
        \item The \textbf{ConcreteSubject}, which maintains the list of Observers and broadcasts notifications.
        \item \textbf{ConcreteObservers}, which implement the \texttt{update()} method to perform their custom action.
    \end{enumerate}
\end{dialogue}

\subsection*{Slide 7: Specific Class Diagram (YouTube Example)}
\begin{dialogue}{Speaker A}
    Applying this to our YouTube problem, we introduce the abstract \texttt{Subscriber} interface as our Observer, which declares a single \texttt{update(channelName, videoTitle)} method. 
    Our \texttt{YouTubeChannel} class implements the Subject interface, maintaining a single list of \texttt{Subscriber*} pointers. Now, \texttt{YouTubeChannel} has no idea whether it's notifying an email subscriber, a mobile app, or a smart TV. It simply calls \texttt{update()} on all registered pointers.
\end{dialogue}

\newpage
\subsection*{Slide 8: Code Implementation \& Demo}
\begin{dialogue}{Speaker B}
    Let's look at the refactored code. The \texttt{YouTubeChannel} class is now clean. To subscribe, we simply push the \texttt{Subscriber*} to the vector; to unsubscribe, we erase it. 
    Adding a new \texttt{TVSubscriber} class requires zero changes to the channel class itself! In our console demo, you can see that when a video is uploaded, all three subscribers receive the updates. When we unsubscribe the mobile app subscriber, the subsequent upload only triggers updates for the email and TV subscribers.
\end{dialogue}

\subsection*{Slide 9: Pros \& Cons}
\begin{dialogue}{Speaker A}
    Like all patterns, the Observer pattern has trade-offs. 
    The main \textbf{Pros} are that it satisfies the Open-Closed Principle and allows dynamic runtime subscriptions. 
    The \textbf{Cons} include the \textbf{Lapsed Listener Problem}---if observers are destroyed without unsubscribing, the subject will hold dead pointers, causing memory leaks or crashes. Additionally, developers must be careful about cascading update loops if observers trigger changes back in the subject.
\end{dialogue}

\subsection*{Slide 10: Other Real-world Applications}
\begin{dialogue}{Speaker B}
    In modern software development, you see this pattern everywhere. 
    In web dev, browser DOM events like \texttt{addEventListener} are classic Observers. Similarly, state management libraries like Redux or Vuex notify UI components to re-render when the state store updates.
    In mobile dev, reactive frameworks like RxSwift, Android LiveData, and SwiftUI's \texttt{@Published} variables use Observers to keep UI views synchronized with underlying data models.
\end{dialogue}

\subsection*{Slide 11: Interactive Quiz}
\begin{dialogue}{Speaker A}
    Now, to wrap things up and keep you all awake, we have a short quiz! Correct answers will get small sweet treats, so raise your hand fast!
\end{dialogue}
\begin{dialogue}{Speaker B}
    {[Reads Q1]} ``What type of design pattern is the Observer Pattern?'' ... {[Wait for hands]} ... Correct! It is Behavioral. Here is your candy!
\end{dialogue}
\begin{dialogue}{Speaker A}
    {[Reads Q2]} ``Which SOLID principle is satisfied when adding new subscribers without changing the channel code?'' ... Yes! Open-Closed Principle. Excellent.
\end{dialogue}
\begin{dialogue}{Speaker B}
    {[Reads Q3]} ``What problem occurs when a listener is deleted without unsubscribing?'' ... Correct, the Lapsed Listener problem leading to memory leaks.
\end{dialogue}

\subsection*{Slide 12: Conclusion \& References}
\begin{dialogue}{Speaker A}
    In conclusion, the Observer pattern is a powerful tool to build scalable, loosely-coupled event-driven systems. Thank you all for listening. We are now open to any questions or reviews from the audience.
\end{dialogue}

\end{document}
