...
To illustrate how the naive solution operates, consider evaluating the string \texttt{"A OR B AND C"} with context $A=\text{true}, B=\text{false}, C=\text{false}$:
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \end{equation*}
        \item \textit{Step 3b:} Reads operator \texttt{"AND"} and operand \texttt{"C"} ($C = \text{false}$). Updates:
        \begin{equation*}
            \text{\texttt{result}} = \text{\texttt{result}} \land C = \text{true} \land \text{false} = \mathbf{false}
        \end{equation*}
    \end{itemize}
\end{enumerate}

\textbf{Critical Flaw Demonstrated:} The naive evaluator evaluates strictly left-to-right as $(A \lor B) \land C = (\text{true} \lor \text{false}) \land \text{false} = \mathbf{false}$. However, according to standard mathematical operator precedence ($\text{AND} > \text{OR}$), the expression must be evaluated as $A \lor (B \land C) = \text{true} \lor (\text{false} \land \text{false}) = \mathbf{true}$. The naive evaluation thus produces an incorrect truth value ($\mathbf{false}$ instead of $\mathbf{true}$) due to its inability to enforce operator precedence and grammatical hierarchy.

\newpage
\section{Analysis of Naive Solution Drawbacks}

The naive string-scanning and procedural dispatch approach presents several architectural, correctness, and performance limitations, summarized in Table~\ref{tab:interpreter_drawbacks}:
...
\label{tab:interpreter_drawbacks}
\end{table}
...
As shown in Figure~\ref{fig:general_uml_structure}, the Interpreter pattern defines a polymorphic hierarchy where non-terminal composite nodes hold references to abstract expressions, while terminal nodes evaluate directly against the context.
...
Figure~\ref{fig:concrete_uml} shows the class hierarchy of our Boolean expression engine. \texttt{BooleanExpression} acts as the abstract component interface with concrete terminals (\texttt{VariableExpression}, \texttt{ConstantExpression}) and non-terminals owning child expressions via \texttt{std::unique\_ptr}.
