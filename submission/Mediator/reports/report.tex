...
% Bottom row: spread out with absolute positioning
...
\node[concrete] (colleague1) at (8, -4) {\textbf{ConcreteColleague1}\\\rule{4.2cm}{0.3pt}\\+ doAction(): void};
\node[concrete] (colleague2) at (14, -4) {\textbf{ConcreteColleague2}\\\rule{4.2cm}{0.3pt}\\+ doAction(): void};
...
\draw[thick] (concreteMediator.east) -- ++(1.0,0) |- (colleague1.west) node[near start, below, font=\scriptsize] {coordinates};
\draw[thick] (concreteMediator.east) -- ++(1.0,0) |- (colleague2.west);
\end{tikzpicture}
}
...
% Association: ATCTower manages Flight
\draw[thick] (atcTower.east) -- ++(1.2,0) |- (flightBase.south east) node[near start, above, font=\scriptsize] {manages 0..*};

% Colleague inheritance: all three inherit from Flight
\draw[->, thick] (commFlight.north) -- (flightBase.south);
\draw[->, thick] (cargoFlight.west) -- ++(-1.0,0) |- (flightBase.south west);
\draw[->, thick] (emergFlight.west) -- ++(-1.5,0) |- ([xshift=-3mm]flightBase.south west);
\end{tikzpicture}
}
