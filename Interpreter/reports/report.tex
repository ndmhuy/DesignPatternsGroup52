...
\subsection{Background: Domain-Specific Languages and Boolean Logic}
...
% Context on the left, Client on the right
\node[class] (context) {\textbf{Context}\\\rule{4.6cm}{0.3pt}\\- stateMap: Map\\+ lookup(key): Value\\+ assign(key, val)};
\node[class, right=1.6cm of context] (client) {\textbf{Client}\\\rule{4.6cm}{0.3pt}\\+ buildAST()\\+ execute()};

% AbstractExpression centered below Client
...
% TerminalExpression (left) and NonterminalExpression (right)
...
% Direct, clean queries-context line on the left side
\draw[->, thick, dashed] (terminal.north) -- (terminal.north |- context.south) node[midway, left, font=\scriptsize] {queries};

% Inheritance tree to AbstractExpression
\draw[thick] (terminal.north) -- ++(0,0.5) coordinate (busL);
\draw[thick] (nonterminal.north) -- ++(0,0.5) coordinate (busR);
\draw[thick] (busL) -- (busR);
\draw[->, thick] (busL -| abstractExp.south) -- (abstractExp.south);

% Nonterminal contains child expressions
...
% Top Center: Abstract BooleanExpression Interface
\node[abstract] (exp) {\textbf{\textit{BooleanExpression}}\\\rule{4.3cm}{0.3pt}\\\textit{+ interpret(Context const\&): bool}};

% Middle Row (3 Classes)
\node[class, below left=1.2cm and 2.6cm of exp] (varExp) {\textbf{VariableExpression}\\\rule{4.3cm}{0.3pt}\\- name\_: string\\+ interpret(Context const\&): bool};
\node[class, right=0.3cm of varExp] (constExp) {\textbf{ConstantExpression}\\\rule{4.3cm}{0.3pt}\\- value\_: bool\\+ interpret(Context const\&): bool};
\node[opclass, right=0.3cm of constExp] (andExp) {\textbf{AndExpression}\\\rule{4.3cm}{0.3pt}\\- left\_, right\_: unique\_ptr\\+ interpret(Context const\&): bool};

% Bottom Row (3 Classes)
\node[opclass, below=0.6cm of varExp] (orExp) {\textbf{OrExpression}\\\rule{4.3cm}{0.3pt}\\- left\_, right\_: unique\_ptr\\+ interpret(Context const\&): bool};
\node[opclass, below=0.6cm of constExp] (notExp) {\textbf{NotExpression}\\\rule{4.3cm}{0.3pt}\\- operand\_: unique\_ptr\\+ interpret(Context const\&): bool};
\node[opclass, below=0.6cm of andExp] (xorExp) {\textbf{XorExpression}\\\rule{4.3cm}{0.3pt}\\- left\_, right\_: unique\_ptr\\+ interpret(Context const\&): bool};

% Vertical links from bottom row to middle row
\draw[thick] (orExp.north) -- (varExp.south);
\draw[thick] (notExp.north) -- (constExp.south);
\draw[thick] (xorExp.north) -- (andExp.south);

% Vertical stubs from outer middle classes up to horizontal bus line
\draw[thick] (varExp.north) -- ++(0, 0.4) coordinate (busLeft);
\draw[thick] (andExp.north) -- ++(0, 0.4) coordinate (busRight);

% Horizontal bus line connecting all 3 columns
\draw[thick] (busLeft) -- (busRight);

% Single unified generalization arrow pointing directly into BooleanExpression.south
\draw[->, thick] (constExp.north) -- (exp.south);
\end{tikzpicture}
...
Table~\ref{tab:interpreter_execution_trace} summarizes the post-order depth-first traversal and evaluation sequence of the AST nodes:
...
\label{tab:interpreter_execution_trace}
\end{table}
...
The Interpreter pattern is widely applied in software engineering when a system needs to parse, evaluate, or execute domain-specific grammar rules:

\begin{itemize}
    \item \textbf{Domain-Specific Query and Filter Languages:} Database abstraction layers and search systems parse user-defined filter expressions (such as SQL \texttt{WHERE} clauses or custom search query languages) into syntax trees to evaluate records dynamically.
    \item \textbf{Business Rule and Validation Engines:} Enterprise workflow platforms use domain-specific mini-languages to evaluate eligibility criteria, discount pricing tiers, or risk scores defined by non-programmer domain experts at runtime.
    \item \textbf{Spreadsheet Formula Evaluation:} Spreadsheet engines parse formula strings (such as mathematical formulas, logical branching, and cell aggregations) into Abstract Syntax Trees that re-evaluate when referenced cell values change.
    \item \textbf{Template Processing and Formatting Engines:} Document and string template engines interpret dynamic placeholder expressions, conditional blocks, and iterative tags embedded in text or markup.
    \item \textbf{Command-Line and Script Interpreters:} Interactive console utilities and automation engines parse script commands, parameterized flags, and compound logical pipelines into executable syntax structures.
\end{itemize}

\newpage
\section{Observer and Mediator Integration}
...
\subsection{Combining Interpreter with the Observer Pattern}
In a static Interpreter implementation, clients must manually invoke \texttt{interpret()} whenever variable values change. In modern reactive architectures—such as reactive UI frameworks, spreadsheet dependency graphs, or IoT sensor triggers—this workflow is refactored by applying the Observer pattern.

The evaluation \texttt{Context} functions as an observable Subject. When a variable's value is modified (for example, via \texttt{Context::assign("temperature", 92.5)}), the Context publishes a state-change notification. Root expressions and dependent components register as Observers listening for specific variable keys. Upon receiving an update event, they automatically trigger re-interpretation of their AST and emit new evaluation results reactively, eliminating wasteful CPU polling loops.

\subsection{Combining Interpreter with the Mediator Pattern}
...
Colleague components do not interact directly with one another. Instead, they send declarative query or command expressions to a central \texttt{Mediator}. The central Mediator uses an internal Interpreter engine to dynamically parse incoming expressions, evaluate business rules, and check permission predicates before routing actions to target colleagues. This decouples the communication topology into an $O(N)$ star architecture while retaining the flexibility to modify business coordination rules dynamically at runtime.

\section{Interactive Quiz}

\begin{enumerate}
    \item What is the primary intent of the Interpreter Design Pattern?
    \begin{itemize}
        \item[A)] To define a family of algorithms, encapsulate each one, and make them interchangeable.
        \item[B)] Given a language, define a representation for its grammar along with an interpreter that uses the representation to interpret sentences in the language.
        \item[C)] To attach additional responsibilities to an object dynamically without subclassing.
        \item[D)] To ensure a class has only one instance and provide a global access point.
    \end{itemize}

    \item What is the fundamental difference between a \texttt{TerminalExpression} and a \texttt{NonTerminalExpression}?
    \begin{itemize}
        \item[A)] Terminal expressions represent leaf nodes in an AST (e.g., variables, constants) and evaluate directly; NonTerminal expressions represent grammar operators (e.g., AND, OR) containing child sub-expressions.
        \item[B)] Terminal expressions only run in console terminals.
        \item[C)] NonTerminal expressions cannot access the \texttt{Context} symbol table.
        \item[D)] Terminal expressions require multi-threading while NonTerminal expressions are single-threaded.
    \end{itemize}

    \item What is the primary responsibility of the \texttt{Context} object in an Interpreter implementation?
    \begin{itemize}
        \item[A)] To render the GUI window for the user.
        \item[B)] To store global state, symbol table mappings (e.g., variable identifiers to boolean values), and evaluation environment information outside the syntax tree.
        \item[C)] To compile the C++ source code into binary machine code.
        \item[D)] To allocate memory using raw \texttt{malloc} calls.
    \end{itemize}

    \item In modern C++17, why is \texttt{std::unique\_ptr} the preferred smart pointer for holding child expressions in non-terminal AST nodes?
    \begin{itemize}
        \item[A)] Because AST trees exhibit strict single-ownership hierarchy with no sharing between distinct parent nodes, guaranteeing automatic deep deletion with zero reference counting overhead.
        \item[B)] Because \texttt{std::shared\_ptr} cannot hold abstract polymorphic types.
        \item[C)] Because \texttt{std::unique\_ptr} automatically compiles expressions to assembly.
        \item[D)] Because \texttt{std::weak\_ptr} is deprecated in C++17.
    \end{itemize}

    \item Under which scenario is the Interpreter pattern generally NOT recommended?
    \begin{itemize}
        \item[A)] When evaluating simple domain-specific filter queries.
        \item[B)] When implementing dynamic business rule engines.
        \item[C)] When parsing complex, large-scale programming language grammars with hundreds of production rules (where Parser Generators like ANTLR or Bison are more maintainable).
        \item[D)] When parsing boolean logic formulas.
    \end{itemize}

    \item How does integrating the Observer pattern enhance an Interpreter AST evaluation pipeline?
    \begin{itemize}
        \item[A)] It eliminates the need for virtual destructor definitions.
        \item[B)] The \texttt{Context} acts as an observable Subject that automatically triggers re-evaluation of dependent AST root expressions whenever variable values change.
        \item[C)] It converts the AST tree into a linear linked list.
        \item[D)] It enforces alphabetical ordering on terminal variable lookups.
    \end{itemize}
\end{enumerate}

\subsubsection*{Answer Key}
\textbf{1-B, 2-A, 3-B, 4-A, 5-C, 6-B.}
