...
If aircraft coordinate landing authorization directly with one another without a central coordinator (a peer-to-peer mesh architecture, illustrated in Figure~\ref{fig:p2p_mesh}):
\begin{enumerate}
...
Even when implemented using standard object-oriented idioms (proper enums, RAII destructors, encapsulation), the decentralized peer-to-peer approach suffers from fundamental architectural flaws, detailed in Table~\ref{tab:naive_drawbacks}:
...
The pattern replaces a chaotic, many-to-many communication mesh with a clean, one-to-many \textbf{Hub-and-Spoke (Star Topology)}, as illustrated in Figure~\ref{fig:star_topology}.
...
As depicted in Figure~\ref{fig:general_mediator_uml}, the generic Mediator structure decouples colleague objects from one another by funneling all interaction logic through an abstract mediator interface.
...
Figure~\ref{fig:atc_mediator_uml} illustrates the airport runway coordination architecture, where \texttt{ATCTower} serves as the central mediator managing flight registrations, holding queues, and landing clearances across specialized flight colleagues.
