...
\centering
\begin{tikzpicture}[
    node/.style={circle, draw=black, thick, fill=blue!10, minimum size=1.1cm, font=\footnotesize\bfseries},
    conn/.style={<->, thick, red!70!black}
]
\node[node] (A) at (90:2.3cm) {VN123};
\node[node] (B) at (18:2.3cm) {AF456};
\node[node] (C) at (306:2.3cm) {BA789};
\node[node] (D) at (234:2.3cm) {FX902};
\node[node] (E) at (162:2.3cm) {MEDIC1};
...
\begin{enumerate}
    ...
    \item \textbf{Decentralized Consensus Request:} \texttt{VN123} invokes \texttt{requestLandingPermission()}, queries \texttt{AF456} and \texttt{BA789}, finds both in \texttt{IN\_FLIGHT}, and acquires \texttt{ON\_RUNWAY}.
...
\addlinespace
\textbf{Dangling Pointer Vulnerabilities} & If a flight object is destroyed without completing the $N$-way unregistration sweep, remaining flights hold stale pointers. & Dereferencing invalid pointers during peer polling leads to undefined behavior or segmentation faults. \\
\bottomrule
\end{tabular}
...
\begin{tikzpicture}[
    node/.style={circle, draw=black, thick, fill=blue!10, minimum size=1.1cm, font=\footnotesize\bfseries},
    mediator/.style={rectangle, draw=black, thick, fill=orange!15, minimum width=2.8cm, minimum height=1.1cm, font=\bfseries\small},
    conn/.style={<->, thick, blue!80!black}
]
\node[mediator] (ATC) at (0,0) {ATCTower (Mediator)};
\node[node] (A) at (90:2.5cm) {VN123};
\node[node] (B) at (18:2.5cm) {AF456};
\node[node] (C) at (306:2.5cm) {BA789};
\node[node] (D) at (234:2.5cm) {FX902};
\node[node] (E) at (162:2.5cm) {MEDIC1};
...
\subsection{Key Structural Participants}
\begin{itemize}
...
\centering
\begin{tikzpicture}[
    class/.style={rectangle, draw=black, thick, fill=white, text width=4.8cm, minimum height=1.1cm, font=\footnotesize},
    abstract/.style={class, fill=blue!6},
    concrete/.style={class, fill=orange!8}
]
% Mediator Interface (top-left) and Colleague Interface (top-right)
\node[abstract] (mediator) {\textbf{\textit{Mediator}}\\\rule{4.8cm}{0.3pt}\\\textit{+ notify(sender, event): void}};
\node[abstract, right=2.0cm of mediator] (colleague) {\textbf{\textit{Colleague}}\\\rule{4.8cm}{0.3pt}\\- mediator: Mediator\\+ setMediator(Mediator): void};

% Concrete Mediator (bottom-left) and Concrete Colleagues (bottom-right)
\node[concrete, below=1.5cm of mediator] (concreteMediator) {\textbf{ConcreteMediator}\\\rule{4.8cm}{0.3pt}\\- colleague1: Colleague1\\- colleague2: Colleague2\\+ notify(sender, event): void};
\node[concrete, below left=1.5cm and -0.4cm of colleague] (colleague1) {\textbf{ConcreteColleague1}\\\rule{4.4cm}{0.3pt}\\+ doAction(): void};
\node[concrete, below right=1.5cm and -0.4cm of colleague] (colleague2) {\textbf{ConcreteColleague2}\\\rule{4.4cm}{0.3pt}\\+ doAction(): void};

% Generalization arrows
\draw[->, thick] (concreteMediator.north) -- (mediator.south);
\draw[thick] (colleague1.north) -- ++(0,0.5) coordinate (cBusL);
\draw[thick] (colleague2.north) -- ++(0,0.5) coordinate (cBusR);
...
\draw[thick] (concreteMediator.east) -- ++(0.5,0) |- (colleague1.west) node[near start, below, font=\scriptsize] {coordinates};
\draw[thick] (concreteMediator.east) -- ++(0.5,0) |- (colleague2.west);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    class/.style={rectangle, draw=black, thick, fill=white, text width=4.5cm, font=\scriptsize},
    abstract/.style={class, fill=blue!8},
    medclass/.style={class, fill=orange!8},
    colclass/.style={class, fill=green!6}
]
% Mediator Interface (top-left) and Flight Base Class (top-right)
\node[abstract] (iMediator) {\textbf{\textit{IATCMediator}}\\\rule{4.5cm}{0.3pt}\\\textit{+ registerFlight(Flight): void}\\\textit{+ requestLanding(Flight*): bool}\\\textit{+ notifyRunwayClear(Flight*): void}\\\textit{+ broadcastEmergency(Flight*, str): void}};

\node[abstract, right=1.6cm of iMediator] (flightBase) {\textbf{\textit{Flight}}\\\rule{4.5cm}{0.3pt}\\- mediator\_: IATCMediator*\\- flightCode\_: string\\+ requestLanding(): bool\\+ land(): void\\+ receiveMessage(from, msg): void};

% Concrete Mediator: ATCTower (bottom-left)
\node[medclass, below=1.4cm of iMediator] (atcTower) {\textbf{ATCTower}\\\rule{4.5cm}{0.3pt}\\- runwayClear\_: bool\\- currentRunwayUser\_: Flight*\\- landingQueue\_: queue$<$Flight*$>$\\- activeFlights\_: vector$<$shared\_ptr$>$\\+ registerFlight(Flight): void\\+ requestLanding(Flight*): bool\\+ notifyRunwayClear(Flight*): void\\+ broadcastEmergency(Flight*, str): void};

% Concrete Colleagues (bottom-right column)
\node[colclass, below=1.2cm of flightBase] (commFlight) {\textbf{CommercialFlight}\\\rule{4.5cm}{0.3pt}\\- passengerCount\_: int\\+ requestLanding(): bool};

\node[colclass, below=0.5cm of commFlight] (cargoFlight) {\textbf{CargoFlight}\\\rule{4.5cm}{0.3pt}\\- cargoWeightTons\_: double\\+ requestLanding(): bool};

\node[colclass, below=0.5cm of cargoFlight] (emergFlight) {\textbf{EmergencyFlight}\\\rule{4.5cm}{0.3pt}\\+ declareEmergency(reason: str): void};

% Inheritance & Association Links
...
\draw[thick] (atcTower.east) -- ++(0.5,0) |- (flightBase.south) node[near start, above, font=\scriptsize] {manages 0..*};

% Colleague inheritance tree
\draw[->, thick] (commFlight.north) -- (flightBase.south);
\draw[->, thick] (cargoFlight.east) -- ++(0.3,0) |- (flightBase.east);
\draw[->, thick] (emergFlight.east) -- ++(0.6,0) |- (flightBase.east);
\end{tikzpicture}
...
\begin{itemize}
    ...
    \item \textbf{Emergency Priority Overrides:} When \texttt{EmergencyFlight} triggers \texttt{declareEmergency()}, standard traffic is alerted and the emergency flight is granted immediate runway access.
\end{itemize}
...
\begin{enumerate}
    \item What is the primary intent of the Mediator Design Pattern?
    \begin{itemize}
        \item[A)] To dynamically add responsibilities to an object without subclassing.
        \item[B)] To encapsulate how a set of objects interact, replacing $O(N^2)$ direct connections with an $O(N)$ star topology.
        \item[C)] To define a grammar for a language and interpret its sentences.
        \item[D)] To ensure a class has only one instance and provide a global point of access to it.
    \end{itemize}

    \item In a decentralized peer-to-peer system of $N$ aircraft, how many communication channels exist?
    \begin{itemize}
        \item[A)] $O(\log N)$
        \item[B)] $O(N)$
        \item[C)] $\frac{N(N-1)}{2} = O(N^2)$
        \item[D)] $O(2^N)$
    \end{itemize}

    \item How does the Mediator pattern differ fundamentally from the Observer pattern?
    \begin{itemize}
        \item[A)] Mediator is a structural pattern, whereas Observer is behavioral.
        \item[B)] Mediator coordinates bidirectional interactions between colleagues; Observer facilitates one-to-many unidirectional broadcast notifications.
        \item[C)] Observer centralizes business logic, whereas Mediator distributes it.
        \item[D)] There is no difference; they are interchangeable aliases.
    \end{itemize}

    \item What is the primary architectural drawback or risk associated with the Mediator pattern?
    \begin{itemize}
        \item[A)] Excessive memory allocation from dynamic polymorphism.
        \item[B)] Risk of the ConcreteMediator evolving into an unmaintainable, monolithic "God Object".
        \item[C)] Inability to support inheritance in colleague classes.
        \item[D)] Colleague objects becoming tightly coupled to each other.
    \end{itemize}

    \item In modern C++ memory management, why should a Colleague store a raw pointer or \texttt{std::weak\_ptr} to the Mediator rather than a \texttt{std::shared\_ptr}?
    \begin{itemize}
        \item[A)] Because \texttt{std::shared\_ptr} cannot point to abstract interfaces.
        \item[B)] To prevent cyclic reference memory leaks where Mediator and Colleague retain each other indefinitely.
        \item[C)] Because raw pointers execute faster during virtual method dispatch.
        \item[D)] To allow the Colleague to delete the Mediator on destruction.
    \end{itemize}

    \item In modern enterprise ASP.NET Core applications, which library implements the in-process Mediator pattern to decouple HTTP Controllers from Command/Query handlers?
    \begin{itemize}
        \item[A)] Entity Framework Core
        \item[B)] AutoMapper
        \item[C)] MediatR
        \item[D)] SignalR
    \end{itemize}

    \item What occurs in the \texttt{ATCTower} mediator when the current runway user notifies \texttt{notifyRunwayClear()}?
    \begin{itemize}
        \item[A)] The tower immediately crashes due to an empty queue.
        \item[B)] The tower broadcasts an unregister request to all flights.
        \item[C)] The tower pops the next waiting aircraft from the FIFO landing queue and grants it clearance automatically.
        \item[D)] The tower forces all airborne flights to land simultaneously.
    \end{itemize}

    \item When the Mediator pattern is combined with the Interpreter pattern, what role does the Interpreter play?
    \begin{itemize}
        \item[A)] It renders the graphical UI for the air traffic controller.
        \item[B)] It dynamically parses and evaluates domain-specific business rules and priority predicates inside the Mediator.
        \item[C)] It converts C++ code into machine assembly at runtime.
        \item[D)] It serializes flight data into JSON strings for network transmission.
    \end{itemize}
\end{enumerate}

\subsubsection*{Answer Key}
\textbf{1-B, 2-C, 3-B, 4-B, 5-B, 6-C, 7-C, 8-B.}
